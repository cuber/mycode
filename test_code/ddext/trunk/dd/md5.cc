...
namespace dd {
   ...
   */
  void MD5::MD5Init (MD5_CTX *context)      /* context */
  {
   ...
   */
  void MD5::MD5String (char *string, unsigned int stringLen)
  {
    MD5_CTX context;
    unsigned char digest[16];
    
    MD5Init (&context);
    MD5Update (&context, (unsigned char *)string, stringLen);
    MD5Final (digest, &context);
    MD5Print (digest);
  }
  void MD5::MD5Code(char* string, unsigned int stringlen,unsigned char *digest)
  {
    MD5_CTX context;
    MD5Init (&context);
    MD5Update (&context, (unsigned char *)string, stringlen);
  ...
  void MD5::MD5TimeTrial ()
  {
    ...
    MD5Print (digest);
    printf ("Speed = %ld bytes %d second\n",  (long)TEST_BLOCK_LEN * (long)TEST_BLOCK_COUNT,(int)(endTime-startTime));
  }
  /* Digests a reference suite of strings and prints the results.
   */
  void MD5::MD5TestSuite ()
  {
    /*
     MD5String ("");
     MD5String ("a");
   ...
   */
  void MD5::MD5File (char *filename)
  {
    FILE *file;
    MD5_CTX context;
    int len;
   ...
   */
  void MD5::MD5Print (unsigned char digest[16])
  {
    unsigned int i;
    char tmpstring[3];
    ...
    tmpstring[2] = 0;
    for (i = 0; i < 16; i++) {
      sprintf(tmpstring,"%02x", digest[i]);
      strcat((char *) result,tmpstring);
    }
  }
